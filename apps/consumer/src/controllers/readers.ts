...
async function reader() {
    ...
    const groupName = process.env.CONSUMER_GROUP_NAME || "consumer-group:undefined";
    const consumerName = process.env.CONSUMER_NAME || "consumer:undefined";
    ...
    if (stream && Array.isArray(stream) && stream[0] && typeof stream[0] === 'object' && stream[0] !== null && 'messages' in stream[0]) {
        (stream[0] as any).messages.forEach(async (message: any) => {
            ...
            await pingAndAck(message.message.websiteUrl, message.id, message.message.websiteId);
        });
    }
    return stream;
}
